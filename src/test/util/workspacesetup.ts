...
export async function setupWorkspace(bazel: boolean = true): Promise<WorkspaceInfo> {
    ...
    const build1 = buildContents("package1", ["package2", "package3", "package3/package4"], ["test1.ts"]);
...
export function test2b() {
    ...
    const build3 = buildContents("package3", [], ["test3a.ts", "test3b.ts"]); // may need to fix empty deps
...
function buildContents(pkg: string, deps: string[], srcs: string[]) {
